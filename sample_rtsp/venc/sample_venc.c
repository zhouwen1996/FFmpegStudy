...
//就类似与VCL播放时需要SDP文件解码播放
//没有实际从码流中获取的其实可以从sps,pps中获取到的
//而是写死的
		char *pTemp2 = sdpMsg;
...
//重点是对客户端发过来的请求进行解析以及发送response
void * RtspClientMsg(void*pParam)
{
	pthread_detach(pthread_self());//线程分离，线程自己回收自己
	int nRes;
	char pRecvBuf[RTSP_RECV_SIZE];
	RTSP_CLIENT * pClient = (RTSP_CLIENT*)pParam;//RTSP 客户端信息
	memset(pRecvBuf,0,sizeof(pRecvBuf));
	printf("RTSP:-----Create Client %s\n",pClient->IP);
	while(pClient->status != RTSP_IDLE)
	{
		nRes = recv(pClient->socket, pRecvBuf, RTSP_RECV_SIZE,0);//非阻塞的
		//printf("-------------------%d\n",nRes);
		if(nRes < 1)
		{
			//usleep(1000);
...
//这里请求的数据解析出来的就是请求格式 ;方法 URL  消息头 消息体
...
//cseq  在获取和返回的时候都有他，则应该是起校准的作用，就是顺序
		if(strstr(cmdName, "OPTIONS"))//返回当前RTSP支持的关键字
		{
			OptionAnswer(cseq,pClient->socket);
			
		}
		//对比ORTP库传输 时VCL播放需要自己配置SDP文件
		//二RTSP传输 则打开一个串流就可以播放了。就是因为自己配置了SDP
		//在服务器往客服端发送的信息里面就已经包含了SDP了
		else if(strstr(cmdName, "DESCRIBE"))//请求流的SDP
		{
...
void * RtspServerListen(void*pParam)
{
    ...
    while ((s32CSocket = accept(s32Socket, (struct sockaddr*)&addrAccept, &nAddrLen)) >= 0)
    {
		...
		{
			if(g_rtspClients[i].status == RTSP_IDLE)//初始化就是这个状态
			{
...
HI_S32 VENC_Sent(char *buffer,int buflen)
{
	...
	for(is=0;is<MAX_RTSP_CLIENT;is++)
	{
		...
		rtp_hdr->marker    = 0;           //当前这包是不是数据的最后一包
		rtp_hdr->ssrc      = htonl(10);   //信源标记

		if(nAvFrmLen<=nalu_sent_len)//不足一包填充的是NALUhead
		{
		//RTP 报头格式
			rtp_hdr->marker=1;
			...
			nalu_hdr->F=0; //第一个都为0
			nalu_hdr->NRI=  nIsIFrm; //重要性级别
			nalu_hdr->TYPE=  nNaluType;//该帧的类型 0没定义
			//RTP头12字节，NALU 1个字节  数据从13开始
		...
		else if(nAvFrmLen>nalu_sent_len)//整包
		{
			int k=0,l=0;
			...
			while(t<=k)
			{
				...
				else if(t<k && t!=0)//中间包，既不是开头也不是结尾
				{
...
HI_S32 saveStream(VENC_STREAM_S *pstStream)
{
    ...
    {
		if(g_rtspClients[j].status == RTSP_SENDING)//没有处于发送状态就没必要存
		{
		    for (i = 0; i < pstStream->u32PackCount; i++)
		    {
...
{
	int i;
	pthread_t threadId = 0;//首先要开一个线程来监听，之后再开线程来服务

	memset(&g_rtp_playload,0,sizeof(g_rtp_playload));
	strcpy(&g_rtp_playload,"G726-32");//G726 32位的视频编解码，这些代码也是移植过来修改的
	g_audio_rate = 8000;
	memset(&g_rtspClients,0,sizeof(RTSP_CLIENT)*MAX_RTSP_CLIENT);//客户端相应的信息存储
...
HI_VOID* vdRTPSendThread(HI_VOID *p)
{
	while(1)
	{
		if(!list_empty(&RTPbuf_head))//取数据，就是venc模块编码之后存放到这里来
		{
...
//最后windows的VLC是作为客户端的，HI3518E是作为服务器的则服务器先启动
	RtspServer_init();//rtsp相关的初始化
